...
class Sgui extends JFrame implements ActionListener, KeyListener {
	...
	Sgui() throws Exception {
		...
		pannel.setLayout(null); // Layout 설정 x(비활성화)
		...
		setResizable(false); //
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 종료(x)버튼 사용가능상태
		setVisible(true); // 프레임창 보여주기
		//여기까지 swing(프레임 관련 설정)
		...
		// 1회 응답
		client = server.accept(); //한번 요청 받기 가능
	...
	public void keyPressed(KeyEvent e){
		...
		if(e.getKeyCode()==10) //엔터키 입력
		{
			try {
				...
				//SERVER->CLIENT (송신)
				dout.writeUTF(txt1.getText()); //outstream에 입력했던 내용을 그대로 보냄-> client로 보냄
				dout.flush();
